...
import { ArrowLeft, ExternalLink, FileText, CheckCircle, Clock, TrendingUp, AlertCircle, RefreshCw } from "lucide-react";
    ...
    return (
      <SiteLayout>
        <div className="container-wide py-20 text-center">
          ...
          <Button onClick={() => navigate("/invest/portfolio")} variant="outline">
            Return to Portfolio
          </Button>
        </div>
      </SiteLayout>
    ...
    <SiteLayout>
      <div className="bg-muted/30 border-b border-border/50">
        <div className="container-wide py-6">
          <Link to="/invest/portfolio" className="inline-flex items-center text-sm font-medium text-muted-foreground hover:text-foreground transition-colors mb-6">
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                <div className="flex items-center gap-3 mb-2">
                  <Badge variant={investment.status === 'active' ? 'default' : 'secondary'} className="uppercase tracking-wider text-[10px]">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="bg-card border border-border/50 rounded-2xl shadow-sm p-6 overflow-hidden relative">
              ...
              <div className="flex items-center justify-between mb-8">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Real-time tracking of your asset growth</p>
                </div>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-card border border-border/50 rounded-2xl shadow-sm p-5">
              <h4 className="font-serif text-lg font-semibold text-foreground mb-4">Document Hub</h4>
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-3 rounded-xl bg-muted/30 border border-border/50 hover:bg-muted/50 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground uppercase tracking-wider mt-0.5">Official Issuance</p>
                    </div>
                  </div>
                  {investment.status === 'confirmed' || investment.status === 'active' || investment.status === 'completed' ? (
                    <Button variant="ghost" size="icon" asChild>
                      <Link to={`/invest/certificate/${investment.id}`}><ExternalLink className="w-4 h-4" /></Link>
                    </Button>
                  ) : (
                ...
                {investment.payments && investment.payments.map((p: any) => p.proof_url && (
                  <div key={p.id} className="flex items-center justify-between p-3 rounded-xl bg-muted/30 border border-border/50 hover:bg-muted/50 transition-colors">
                    <div className="flex items-center gap-3">
                      <CheckCircle className="w-4 h-4 text-green-500" />
                      <div>
                        <p className="text-sm font-semibold text-foreground">Payment Receipt</p>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-wider mt-0.5">{p.provider}</p>
                      </div>
                    </div>
                    <Button variant="ghost" size="icon" asChild>
                      <a href={p.proof_url} target="_blank" rel="noopener noreferrer"><ExternalLink className="w-4 h-4" /></a>
                    </Button>
                  </div>
                ))}
